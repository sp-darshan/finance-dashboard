  ...
  return (
    <Card>
      <div className="mb-5 flex items-center justify-between gap-4">
        <div>
          <h2 className="text-lg font-semibold text-white">Balance trend</h2>
      ...
      <div className="overflow-x-auto pt-4">
        <svg viewBox={`0 0 ${width} ${height}`} className="min-w-full">
          <line x1={padding} y1={midY} x2={width - padding} y2={midY} stroke="rgba(113,113,122,0.28)" />
          <line x1={midX} y1={padding} x2={midX} y2={height - padding} stroke="rgba(113,113,122,0.28)" />
          {[0, 1, 2, 3, 4].map((line) => (
            <line
              key={line}
              x1={padding}
          ...
          {points.map((point) => (
            <g key={point.key}>
              {renderValueLabel(point, midX, midY)}
              <circle cx={point.x} cy={point.y} r="5" fill="#fafafa" stroke="#f59e0b" strokeWidth="1.5" />
...
function renderValueLabel(point, midX, midY) {
  ...
  if (isTopHalf && isRightHalf) {
    return (
      <g>
        ...
        />
        <text x={point.x - 8} y={point.y + 27} textAnchor="end" className="fill-zinc-100 text-[13px] font-semibold">
          {value}
        </text>
      </g>
    );
    ...
    return (
      <g>
        <rect
          ...
          stroke="rgba(255, 255, 255, 0.12)"
        />
        <text x={point.x} y={point.y - 19} textAnchor="middle" className="fill-zinc-100 text-[13px] font-semibold">
          {value}
        </text>
      </g>
    );
  ...
  return (
    <g>
      <rect
        ...
        stroke="rgba(255, 255, 255, 0.12)"
      />
      <text x={point.x + 14} y={point.y + 27} textAnchor="start" className="fill-zinc-100 text-[13px] font-semibold">
        {value}
      </text>
    </g>
  );
